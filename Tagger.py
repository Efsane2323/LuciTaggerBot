...
async def help(event):
  helptext = "**Çok özellikleri Etiket Botu Bulmaya Çalışan Grub Sahibleri @Tegger_utag_bot Size Göre:\n\n📌 5-li etiket\n📌 Emoji etiket\n📌 Tekli Etiket\n📌 Yalnız Yöneticileri etiketleme\n📌\n\n Böyle Çok özellikli @Tegger_utag_bot 'u grubunuza yönetici olarak ekleyip rahatlıkla üyelir , etiket ata bilirsiz **"
  await event.reply(helptext,
                    buttons=(
...
async def mentionall(event):
  ...
  if mode == "text_on_reply":
    ...
    async for usr in client.iter_participants(event.chat_id):
      ...
      usrtxt += f"[{random.choice(emoji)}](tg://user?id={usr.id}) "
      if event.chat_id not in anlik_calisan:
        await event.respond("Işlem Başarıyla Durduruldu\n\n**Buda sizin reklamınız ola bilir @hababammsinifii**❌")
        return
      if usrnum == 5:
...
print(">> Bot çalıyor merak etme https://t.me/hababammsinifii🚀  bilgi alabilirsin <<")
